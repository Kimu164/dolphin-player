		...
		if (frames>25) {
			...
			if (dt >= 5.0)
			{
...
#endif
				{
					...
					__android_log_print(ANDROID_LOG_INFO, "AV FPS", "%s",log_msg);
...
static int try_to_set_best_video_mode(int w, int h, int rgb565)
{
	...
	int flags = SDL_HWSURFACE | SDL_ASYNCBLIT | SDL_HWACCEL | 
			SDL_RESIZABLE | SDL_FULLSCREEN | SDL_NOFRAME;
	int bpp;
...
do_seek:
			...
			case FF_GUI_ASPECT_RATIO_EVENT:
				{
					__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Aspect Ratio Received:%p", event.user.data1);
					{ 
			...
			case FF_GUI_SEEK_EVENT:
				{
					__android_log_print(ANDROID_LOG_INFO, "BroovPlayer", "Seek Value Received:%p", event.user.data1);
					if (g_total_duration) {
...
static int decode_thread(void *arg)
{
	...
	while (1) {
		...
		if (ret < 0)  {
			if (ret == AVERROR_EOF || url_feof(pFormatCtx->pb)  || pFormatCtx->pb->eof_reached == 1)
				g_eof=1;
