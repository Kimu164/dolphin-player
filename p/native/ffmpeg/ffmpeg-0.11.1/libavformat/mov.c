...
static int mov_read_ftyp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    avio_read(pb, type, 4);
    if (strcmp(type, "qt  ")|| strcmp(type, "sdv "))
        c->isom = 1;
    ...
    avio_read(pb, comp_brands_str, comp_brand_size);
	 if (strcmp(type, "sdv ")){
        memcpy(comp_brands_str,"qt  ",4);
    }
    comp_brands_str[comp_brand_size] = 0;
    if (strcmp(type, "sdv ")){
        av_dict_set(&c->fc->metadata, "compatible_brands", "qt  ", 0);
    }else{
...
int ff_mov_read_stsd_entries(MOVContext *c, AVIOContext *pb, int entries)
{
	...
	AVDictionaryEntry *compatible_brands = av_dict_get(c->fc->metadata, "major_brand", NULL, AV_DICT_MATCH_CASE);
    ...
    for (pseudo_stream_id=0; pseudo_stream_id<entries; pseudo_stream_id++) {
        ...
        if (strstr(compatible_brands->value, "sdv ") && st->codec->codec_type==AVMEDIA_TYPE_VIDEO ){
            format = 0x31637661; //avc1
        }
    	if(format == MKTAG(0x9E, 0x89, 0x9C, 0xCE)){
    		format = 0x31637661; //avc1
    	}
    	if(format == MKTAG(0x92, 0x8F, 0xCB, 0x9E)){
    		format = MKTAG('m', 'p', '4', 'a'); //mp4a
    	}
		
    	av_log(c->fc, AV_LOG_DEBUG, "format = %"PRIu32" \n", format);
        if (size >= 16) {
...
static const AVClass class = {"mov,mp4,m4a,3gp,3g2,mj2,sd1", av_default_item_name, options, LIBAVUTIL_VERSION_INT};


AVInputFormat ff_mov_demuxer = {
    .name           = "mov,mp4,m4a,3gp,3g2,mj2,sd1",
